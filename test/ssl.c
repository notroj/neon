...
static int pkcs11_uri(void)
{
    ...
    ret = ne_ssl_pkcs11_uri_provider_init(&prov, TEST_URI, 0);
    if (ret) {
        if (ret == NE_PK11_NOTIMPL)
            t_context("PKCS#11 URI provider not supported");
        else
            t_context("error loading PKCS#11 URI provider");
        return SKIP;
    }

    pind.password[0] = "foobar";
    pind.password[1] = NULL;
    pind.trace = ne_buffer_create();

    ne_ssl_pkcs11_provider_pin(prov, pkcs11_pin, &pind);
    ne_ssl_set_pkcs11_provider(sess, prov);
    ...
    if (ret == OK)
        ONCMPN("pin(0,[none],[none],0)\n", pind.trace->data, "pin callback", "data");
    ne_buffer_destroy(pind.trace);
